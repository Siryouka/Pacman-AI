...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for i in range(0, len(self.allPositions)):
            ...
            distD[position] = self.beliefs[position] * prob
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(0, len(self.allPositions)):
            oldP = self.allPositions[i]
            newPositionDist = self.getPositionDistribution(gameState, oldP)
            oldPr = self.beliefs[oldP]
            for newPosition in newPositionDist.keys():
                distD[newPosition] = oldPr * newPositionDist[newPosition] + distD[newPosition]
...
class JointParticleFilter(ParticleFilter):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            prevGP = list(oldParticle)
            for i in range(self.numGhosts):
                if (oldParticle, i) in cache:
                    newParticle[i] = cache[(oldParticle, i)].sample()
                else:
                    newPosDist = self.getPositionDistribution(gameState, prevGP, i, self.ghostAgents[i])
